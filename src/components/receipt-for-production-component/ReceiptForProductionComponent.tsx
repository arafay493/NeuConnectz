// Note: Receipt From Production Component...!
...
import { fetchRecieptFromProductionList } from '@/redux/actions/sap-actions/sap-actions';
...
const ReceiptFromProduction: FC<ApiProp> = ({ apiUrl }) => {
    ...
    const { recieptFromProductionList, recieptFromProductionCount } = useAppSelector(({ sapStates }) => { return sapStates });
    // console.log("Reciept From Production List:", recieptFromProductionList);
    ...
    const columns = useMemo<ColumnDef<RecieptFromProductionDataType>[]>(
        () => [
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Doc No', (recieptFromProductionList || []).map(item => String(item.documentNumber)), 150, 220),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Item Code', (recieptFromProductionList || []).map(item => item.itemCode), 150, 220),
            },
            {
                accessorKey: '',
                header: 'Item Desc',
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Quantity', (recieptFromProductionList || []).map(item => String(item.quantity)), 120, 150),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('WHS', (recieptFromProductionList || []).map(item => item.wareHouseCode), 150, 220),
            },
            {
                accessorKey: '',
                header: 'Base Qty',
    ...
    const table = useReactTable({
        data: recieptFromProductionList || [], // Handle undefined/null case
        columns,
        ...
        pageCount: Math.ceil(recieptFromProductionCount / pagination.pageSize), // Calculate total pages from server data
        state: {
            sorting,
            globalFilter,
            columnFilters,
    ...
    useEffect(() => {
        if (authenticatedUser?.token) {
            ...
            dispatch(fetchRecieptFromProductionList({
            ...
            >
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            Showing {skipRecord + 1} to {Math.min(skipRecord + pagination.pageSize, recieptFromProductionCount)} of {recieptFromProductionCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box>
...
export default memo(ReceiptFromProduction);