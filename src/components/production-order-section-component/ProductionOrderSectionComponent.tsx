...
import { ActionIcon, Box, Group, Image, Stack, Text, Title, Select, Button } from '@mantine/core';
...
import { fetchAllProductionOrders, fetchProductionOrdersLinesList } from '@/redux/actions/sap-actions/sap-actions';
...
const ProductionOrderSectionComponent: FC<ApiProp> = ({ apiUrl }) => {
    ...
    const { productionOrdersList, productionOrdersCount } = useAppSelector(({ sapStates }) => { return sapStates });
    ...
    const getProductionOrderLinesList = (rowData: ProductionOrderDataType) => {
        ...
        if (rowData && authenticatedUser?.token) {
            dispatch(fetchProductionOrdersLinesList({
    ...
    const columns = useMemo<ColumnDef<ProductionOrderDataType>[]>(
        () => [
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Doc Num', (productionOrdersList || []).map(item => String(item.documentNumber)), 150, 220),
            },
            {
                accessorKey: 'itemNo',
                header: 'Item Code',
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Item Code', (productionOrdersList || []).map(item => item.itemNo), 150, 220),
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Item Description', (productionOrdersList || []).map(item => new Date(item.productDescription).toLocaleDateString()), 150, 220),
            },
            {
                accessorKey: 'uom',
                header: 'UOM',
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('UOM', (productionOrdersList || []).map(item => item.uom), 150, 220),
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Quantity', (productionOrdersList || []).map(item => String(item.quantity)), 120, 150),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Remaining Quantity', (productionOrdersList || []).map(item => String(item.remainingQuantity)), 150, 220),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Planned Date', (productionOrdersList || []).map(item => item.plannedDate), 150, 220),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Origin No', (productionOrdersList || []).map(item => String(item.originNo)), 150, 220),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Warehouse', (productionOrdersList || []).map(item => item.warehouse), 150, 220),
            },
            {
                accessorKey: 'productionOrderStatus',
                header: 'Status',
                cell: ({ getValue, row }) => {
                    const val = row.original.productionOrderStatus?.replace('bopos', '') || 'N/A';
                    return (
                        <Text c={customStyles.colors._909090} fw={500}>
                            {val as string}
                        </Text>
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Status', (productionOrdersList || []).map(item => item.productionOrderStatus), 150, 180),
            },
            {
                header: 'Action',
                cell: ({ getValue, row }) => {
    ...
    useEffect(() => {
        if (isTableModalOpen === false) store.dispatch(CLEAR_ALL_PRODUCTION_ORDERS_LINES_DATA());
    }, [isTableModalOpen]);
            ...
            >
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            Showing {skipRecord + 1} to {Math.min(skipRecord + pagination.pageSize, productionOrdersCount)} of {productionOrdersCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box>
