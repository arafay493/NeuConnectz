...
const PostedGRNTable = () => {
  ...
  const { vendorCodeList } = useAppSelector(({ sapStates }) => {
    return sapStates;
  });
  const { list_Integrated_GRNS_Data, totalGRNS_DataCounts, sapErrorState } =
    ...
    () => [
      ...
      },
      {
        ...
        size: calculateColumnWidth(
          "Doc No",
          list_Integrated_GRNS_Data.map((item) => String(item.docNum)),
          120,
          200
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Vendor Code",
          list_Integrated_GRNS_Data.map((item) => item.vendorCode),
          120,
          200
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Warehouse",
          list_Integrated_GRNS_Data.map((item) => item.whsCode),
          120,
          200
        ),
      },
      ...
      },
      {
        ...
        size: calculateColumnWidth(
          "Doc Status",
          list_Integrated_GRNS_Data.map((item) => item.docStatus),
          120,
          180
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Item Code",
          list_Integrated_GRNS_Data.map((item) => item.itemCode),
          150,
          250
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Item Name",
          list_Integrated_GRNS_Data.map((item) => item.itemName),
          150,
          250
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Group Code",
          list_Integrated_GRNS_Data.map((item) => item.groupCode),
          200,
          220
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Group Name",
          list_Integrated_GRNS_Data.map((item) => item.groupName),
          200,
          300
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "Quantity",
          list_Integrated_GRNS_Data.map((item) => String(item.quantity)),
          200,
          300
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "ERP Doc Entry",
          list_Integrated_GRNS_Data.map((item) =>
            item.erpDocEntry ? String(item.erpDocEntry) : "-"
          ),
          140,
          220
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "ERP Line ID",
          list_Integrated_GRNS_Data.map((item) =>
            item.erpDocLine ? String(item.erpDocLine) : "-"
          ),
          130,
          200
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "SAP Status",
          list_Integrated_GRNS_Data.map(
            (item) => item.sapStatus || item.vendorCode
          ),
          120,
          180
        ),
      },
      {
        ...
        size: calculateColumnWidth(
          "User Name",
          list_Integrated_GRNS_Data.map((item) => item.userName),
          150,
          250
        ),
      },
  ...
  const selectVendorCodesData = vendorCodeList.map((vendor) => ({
  ...
  const handleExportToCSV = () => {
    ...
    let apiUrl = !isFilterParams
      ? process.env.NEXT_PUBLIC_EXPORT_GRN_TO_EXCEL
      : `${process.env.NEXT_PUBLIC_EXPORT_GRN_TO_EXCEL}?${isFilterParams}`;
    dispatch(
      exportDataToCsvFile({
        token: authenticatedUser?.token || "",
        apiUrl: apiUrl || "",
          ...
          <Group gap="md" align="center">
            ...
            <Text size="sm" c={customStyles.colors._909090}>
              ...
              {Math.min(
                ...
                integratedGRNDataCount
              )}{" "}
              of {integratedGRNDataCount} entries
            </Text>
          </Group>
        </Group>
      </Box>
