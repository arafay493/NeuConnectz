...
const UnpostedGRNTable = () => {
    ...
    const columns = useMemo<ColumnDef<GRN_Props>[]>(
        () => [
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Number', list_Pending_GRNS_Data.map(item => String(item.docNum)), 120, 200),
            },
            {
                accessorKey: 'whsCode',
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Warehouse', list_Pending_GRNS_Data.map(item => item.whsCode), 120, 200),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Vendor', list_Pending_GRNS_Data.map(item => item.vendorCode), 120, 200),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Doc Status', list_Pending_GRNS_Data.map(item => item.docStatus), 120, 180),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Item Code', list_Pending_GRNS_Data.map(item => item.itemCode), 150, 250),
            },
            {
...

            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('ERP Doc Entry', list_Pending_GRNS_Data.map(item => item.erpDocEntry ? String(item.erpDocEntry) : '-'), 140, 220),
            },
            {
                ...
                filterFn: numberFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('ERP Line ID', list_Pending_GRNS_Data.map(item => item.erpDocLine ? String(item.erpDocLine) : '-'), 130, 200),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('SAP Status', list_Pending_GRNS_Data.map(item => item.sapStatus || item.vendorCode), 120, 180),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('User Name', list_Pending_GRNS_Data.map(item => item.userName), 150, 250),
            },
            {
    ...
    const buildFilterParams = () => {
        ...
        params.append('sapStatus', 'pending'); // Always include SAP status
    ...
    const fetchFilteredData = () => {
        ...
        setIsLoading(true);
        dispatch(fetchAll_PENDING_GRNS({
    ...
    }, [vendorCode, docStatus, warehouseCode, selectDate, pagination.pageIndex, pagination.pageSize, authenticatedUser]); // Add pagination dependencies
            ...
            >
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            Showing {skipRecord + 1} to {Math.min(skipRecord + pagination.pageSize, pendingGRNDataCount)} of {pendingGRNDataCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box >
...
export default UnpostedGRNTable