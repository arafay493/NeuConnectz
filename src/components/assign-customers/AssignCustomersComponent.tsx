// Note: Assign Customers Component...!
...
import {
    AssignGroupToUserDataType,
    GroupCodeDataType,
} from "@/types/modules/group-types/group-types";
import {
    ActionIcon,
    Box,
    Button,
...
const AssignCustomersComponent = () => {
    ...
    const [groupPermission, setGroupPermission] =
        useState<AssignGroupToUserDataType>();
    ...
    const {
        ListAllGroupCodes: { groups, totalCount: groupsTotalCount },
    ...
    const handlePermissionChange = useCallback(
        (groupId: string, permission: "allow", checked: boolean) => {
            setGroupPermission((prev) => {
                ...
                const existingIndex = prev.groupCodes.findIndex(
                    (code) => code === groupId
                );
    ...
    const handleSelectAllCurrentPage = useCallback(
        (checked: boolean, currentPageData: GroupCodeDataType[]) => {
            if (!selectedUser) return;

            const currentPageGroupCodes = currentPageData.map(
                (item: GroupCodeDataType) => String(item.groupCode)
            );
            ...
            setGroupPermission((prev) => {
                ...
                if (checked) {
                    ...
                    const filteredGroupCodes = existingGroupCodes.filter(
                        (code: string | number) =>
        ...
        () => [
            ...
            },
            {
                ...
                header: ({ table }) => {
                    ...
                    const currentPageData = currentPageRows.map(
                    ...
                    const allCurrentPageSelected =
                        ...
                        currentPageGroupCodes.every((code: string) =>
                            groupPermission?.groupCodes?.includes(code)
                        );
                    const someCurrentPageSelected = currentPageGroupCodes.some(
                        (code: string) => groupPermission?.groupCodes?.includes(code)
                    );
                ...
                cell: ({ row }) => {
                    ...
                    const isChecked =
                        groupPermission?.groupCodes?.includes(String(groupId)) || false;
    ...
    useEffect(() => {
        if (selectedUser) {
            if (listGroupCodesByUserId && listGroupCodesByUserId.length > 0) {
                const groupCodes = listGroupCodesByUserId.map((group) =>
    ...
    const handleResponse = (response: any): void => {
        if (response && response.status == 201) {
            showNotificationToast(
                ...
                "Requested groups has been assigned to the requested user",
        ...
        if (response && response.status != 201) {
            showNotificationToast(
                ...
                "Requested groups could not be assigned to the requested user",
    ...
    return (
        <Box>
            ...
            >
                Select user to assign customers
            </Text>
            ...
            >
                ...
                >
                    ...
                    <Button
                        ...
                        mt={isSmallScreen ? 16 : 0}
                    >
                        {isSmallScreen ? "Assign" : "Assign Customers"}
                    </Button>
                </Group>
            </Group>
                ...
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            ...
                            {Math.min(skipRecord + pagination.pageSize, groupsTotalCount)} of{" "}
                            {groupsTotalCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box>
...
export default AssignCustomersComponent;