// Note: Issue For Production Component...!
...
const IssueForProductionComponent: FC<ApiProp> = ({ apiUrl }) => {
    ...
    const columns = useMemo<ColumnDef<IssuesForProductionDataType>[]>(
        () => [
            {
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Doc No', (issuesForProductionList || []).map(item => String(item.documentNumber)), 150, 220),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Item Code', (issuesForProductionList || []).map(item => item.itemCode), 150, 220),
            },
            {
            ...
            },
            {
                ...
                enableColumnFilter: true,
                size: calculateColumnWidth('Quantity', (issuesForProductionList || []).map(item => String(item.quantity)), 120, 150),
            },
            {
                ...
                filterFn: stringFilterFn,
                enableColumnFilter: true,
                size: calculateColumnWidth('Warehouse', (issuesForProductionList || []).map(item => item.wareHouseCode), 150, 220),
            },
            {
    ...
    const table = useReactTable({
        data: issuesForProductionList || [], // Handle undefined/null case
        columns,
        ...
        pageCount: Math.ceil(issuesForProductionCount / pagination.pageSize), // Calculate total pages from server data
        state: {
            sorting,
            globalFilter,
            columnFilters,
    ...
    useEffect(() => {
        if (authenticatedUser?.token) {
            ...
            dispatch(fetchIssuesForProductionList({
            ...
            >
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            Showing {skipRecord + 1} to {Math.min(skipRecord + pagination.pageSize, issuesForProductionCount)} of {issuesForProductionCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box>
...
export default memo(IssueForProductionComponent);