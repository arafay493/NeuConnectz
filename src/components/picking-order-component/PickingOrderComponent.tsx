...
import PickingOrderFiltersSection from './PickingOrderFilterSection';
import PickingPostedComponent from './PickingPostedComponent';
import PickingUnPostedComponent from './PickingUnPostedComponent';

const PickingOrderComponent = () => {
    ...
    return (
        <Box>
            ...
            {/* Custom Tab Panels */}
            <div>
                {tab === 'Posted' && (<PickingPostedComponent reservationApiUrl = "/IReservationFeature/ListAllReservations?sapStatus=Integrated" outboundApiUrl = "/IStockTransferOrderFeature/ListAllStos?sapStatus=Integrated" salesOrderApiUrl = "/ISalesOrderFeature/GetDetailsOfSalesOrder?sapStatus=Integrated" apiUrl={`${process.env.NEXT_PUBLIC_FETCH_ALL_GOOD_ISSUES}?sapStatus=pending` as string} />)}
                {tab === 'Unposted' && (<PickingUnPostedComponent reservationApiUrl = "/IReservationFeature/ListAllReservations?sapStatus=pending" outboundApiUrl = "/IStockTransferOrderFeature/ListAllStos?sapStatus=pending" salesOrderApiUrl = "/ISalesOrderFeature/GetDetailsOfSalesOrder?sapStatus=pending" apiUrl={`${process.env.NEXT_PUBLIC_FETCH_ALL_GOOD_ISSUES}?sapStatus=Integrated` as string} />)}
            </div>
        </Box>
    );
};

export default memo(PickingOrderComponent);