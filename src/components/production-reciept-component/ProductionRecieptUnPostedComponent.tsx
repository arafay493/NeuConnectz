...
import { fetchListAllProductionReciept, confirmProductionReciept, postProductionReciept, deleteProductionReciept} from '@/redux/actions/production-reciept-actions/production-reciept-actions';
...
import ProductionRecieptUnPosted_Columns from '../columns/ProductionRecieptUnPosted_Columns';

type ApiProp = {
    apiUrl: string;
};

const ProductionRecieptUnPostedComponent: FC<ApiProp> = ({ apiUrl }) => {
    ...
    const handleConfirm = () => {
        ...
        dispatch(confirmProductionReciept({
            payload: {
    ...
    const handlePost = (rowData: any) => {
        handleModalClose()
        setIsFullPageLoading(true)
        dispatch(postProductionReciept({
            payload: {
    ...
    const handleDelete = () => {
        handleModalClose()
        setIsFullPageLoading(true)
        dispatch(deleteProductionReciept({
            apiUrl: `/IProductionReceiptFeature/DeleteProductionReceipt?docNum=${selectedRow?.docNum}`,
            token: authenticatedUser?.token || '',
    ...
    const columns = ProductionRecieptUnPosted_Columns({
    ...
    const handleInBoundStoResponse = (status: number, data: any, error: string) => {
        if (status === 200) {
            showNotificationToast("Confirm Production Receipt", data.message, customStyles.colors._408CCE);
        } else if (error) {
    ...
    const handlePostInBoundStoResponse = (status: number, message: string, error: string) => {
        if (status === 201) {
            ToastMessage("Production Receipt Post", message, status, error)
        } else {
    ...
    const handleDeleteResponse = (status: number, message: string, error: string) => {
        if (status === 200) {
            ToastMessage("Production Receipt Delete", message, status, error)
        } else {
            ToastMessage("Error", message, status, error)
        }
    }
    ...
    return (
        ...
        <Stack>
            ...
            <TanStackTable
                ...
                title={"Unposted Production Receipt"}
                subTitle={"Track and review production receipt seemlessly."}
...
export default memo(ProductionRecieptUnPostedComponent);