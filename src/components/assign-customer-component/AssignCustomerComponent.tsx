...
import TanStackTable from "./TStackTable";
import PlantsList_Columns from "./AssignCustomerColumns";
import showNotificationToast from "@/lib/notification-toast/notification-toast";
import Loader from "../loader/loader";
import { assignPlantsToUser, fetchListAllPlantsCodes, fetchListAllUserPlantsCodes } from "@/redux/actions/customer-user-actions/customer-user-actions";
import { CLEAR_ALL_PLANTS_STATES_BY_USER } from "@/redux/reducers/customer-user-reducer/customer-user-reducer";
...
const AssignPlantsComponent = () => {
    ...
    } = useAppSelector(({ customerUserStates }) => customerUserStates);
    const {
    ...
    const handleResponse = (data: any) => {
        ...
        showNotificationToast("Customer Assigned", data.message, customStyles.colors._408CCE);
        dispatch(
            fetchListAllPlantsCodes({
                authToken: authenticatedUser?.token as string,
                lastCount: pagination.pageSize, // Use page size for server-side pagination
    ...
    const handleAssignPlants = () => {
        ...
        const payload = {
            userId: selectedUser ?? null,
            customerCodes: selectedPlants.map((plant: any) => plant.customerCode)
        }
        dispatch(
            assignPlantsToUser({
    ...
    return (
      ...
      />} */}
            ...
            <TanStackTable
                ...
                subTitle={"Select user to assign customer"}
                skipRecord={skipRecord}
            />
        </Box>
    );
