...
interface ListAllItemsByUidTableModalProps {
    open: boolean,
    close: () => void,
...
interface ListAllMasterItemsByUidProps {
    createdBy: string,
    updatedBy: string,
    createdDate: string,
    updatedDate: string,
...
const ListAllItemsByUidTableModal: FC<ListAllItemsByUidTableModalProps> = ({ open, close, customerReferenceId, deliveryDate, whsCode }) => {
    ...
    const columns = useMemo<ColumnDef<ListAllMasterItemsByUidProps>[]>(
        () => [
            {
                id: 'select',
                header: '',
            ...
            },
            {
                ...
                size: calculateColumnWidth('Item Code', (listAllItemsByUid || []).map(item => item.itemCode), 150, 200),
            },
            {
                ...
                size: calculateColumnWidth('Item Name', (listAllItemsByUid || []).map(item => item.itemName), 150, 150),
            },
            {
                accessorKey: 'uom',
                ...
                size: calculateColumnWidth('Unit Of Measure', (listAllItemsByUid || []).map(item => item.uom), 200, 200),
            },
            {
                accessorKey: 'canQTY',
                header: 'Quantity',
                ...
                size: calculateColumnWidth('Quantity', (listAllItemsByUid || []).map(item => item.canQTY), 200, 200),
            }
        ],
        [listAllItemsByUid, targetRow] // Add data as dependency to recalculate when data changes
    );
    ...
    const table = useReactTable({
        ...
        pageCount: Math.ceil(listAllItemsByUidCount / pagination.pageSize), // Calculate total pages from server data
        state: {
            sorting,
            globalFilter,
            columnFilters,
    ...
    // Note: Fetch all list items by uid...!
    const fetchAllListAllItemsByUid = async () => {
        try {
    ...
    useEffect(() => {
        if (authenticatedUser) {
            setIsLoading(true);
            fetchAllListAllItemsByUid();
        };
    }, [authenticatedUser, dispatch, pagination.pageIndex, pagination.pageSize, open == true]);
    ...
    const createSaleOrder = async () => {
        ...
        try {
            ...
            if (status == 201) {
                ...
                setTargetRow(null);
                close();
                showNotificationToast("Success", "Sale Order Created Successfully", customStyles.colors._1B59F8);
            };
        }

        catch (error) {
            console.log('Something went wrong while creating sale order:', error);
        };
    };
                ...
                >
                    <Group justify="space-between" align="center">
                        ...
                        <Group gap="md" align="center">
                            ...
                            <Text size="sm" c={customStyles.colors._909090}>
                                Showing {(pagination.pageIndex * pagination.pageSize) + 1} to {Math.min((pagination.pageIndex + 1) * pagination.pageSize, listAllItemsByUidCount)} of {listAllItemsByUidCount} entries
                            </Text>
                        </Group>
                    </Group>
                </Box>
...
export default memo(ListAllItemsByUidTableModal);