// Note: Add delivery order drafted table component...!
...
interface DraftedDeliveryOrderItemsProps {
    createdBy: string,
    updatedBy: string,
    createdDate: string,
    updatedDate: string,
...
interface AddDeliveryOrderDraftedTableProps {
    openCompletedSOItemsModal: () => void,
    whsCode: string,
    vehicleNumber : string,
...
const AddDeliveryOrderDraftedTable: FC<AddDeliveryOrderDraftedTableProps> = ({
    openCompletedSOItemsModal,
    // DeliveryDate,
    // customerReferenceId,
    whsCode,
    vehicleNumber,
    ...
    const [draftedDeliveryOrderItems, setDraftedDeliveryOrderItems] = useState<DraftedDeliveryOrderItemsProps[]>([]);
    const [draftedDeliveryOrderItemsCount, setDraftedDeliveryOrderItemsCount] = useState<number>(0);
    ...
    const columns = useMemo<ColumnDef<DraftedDeliveryOrderItemsProps>[]>(
        () => [
            {
                header: 'S.No',
                cell: ({ row }) => {
            ...
            },
            {
                ...
                size: calculateColumnWidth('Item Code', (draftedDeliveryOrderItems || []).map(item => item.itemCode), 150, 200),
            },
            {
                ...
                size: calculateColumnWidth('Item Name', (draftedDeliveryOrderItems || []).map(item => item.itemName), 150, 150),
            },
            {
                accessorKey: 'uoM',
                ...
                size: calculateColumnWidth('Unit Of Measure', (draftedDeliveryOrderItems || []).map(item => item.uoM), 200, 200),
            },
            {
                ...
                size: calculateColumnWidth('Quantity', (draftedDeliveryOrderItems || []).map(item => item.quantity), 200, 200),
            }
        ],
        [draftedDeliveryOrderItems] // Add data as dependency to recalculate when data changes
    );
    ...
    const table = useReactTable({
        data: draftedDeliveryOrderItems,
        columns,
        ...
        pageCount: Math.ceil(draftedDeliveryOrderItemsCount / pagination.pageSize), // Calculate total pages from server data
        state: {
            sorting,
            globalFilter,
            columnFilters,
    ...
    // Note: Fetch list all drafted delivery order items...!
    const fetchListAllDraftedDeliveryOrderItems = async () => {

        try {
            const response = await apiGet(`/neu-connect/v2${process.env.NEXT_PUBLIC_LIST_ALL_DRAFTED_DELIVERY_ORDER_ITEMS}?whsCode=${whsCode}&VehicleNumber=${vehicleNumber}`, authenticatedUser?.token);
            // console.log('Drafted DO Items List: ', response);

            const { status, data } = response;
            if (status == 200) {
                setDraftedDeliveryOrderItems(data?.data?.data || []);
                setDraftedDeliveryOrderItemsCount(data?.data?.totalRecords || 0);
            ...
            console.log('Something went wrong while fetching list all drafted delivery order items', error);
            setIsLoading(false);
            // draftedData([]);
        };
    };
    ...
    useEffect(() => {
        if (authenticatedUser && whsCode && vehicleNumber) {
            setIsLoading(true);
            fetchListAllDraftedDeliveryOrderItems();
        }

        else {
            setDraftedDeliveryOrderItems([]);
            setDraftedDeliveryOrderItemsCount(0);
            // draftedData([]);
        }
    }, [authenticatedUser, pagination.pageIndex, pagination.pageSize, whsCode, vehicleNumber]); // Add pagination dependencies for server-side pagination
        ...
        <Paper shadow="md" radius="md" p="xl" withBorder mt={'2%'}>
            <Group justify="space-between" align="center" style={{ flexShrink: 0, marginBottom: '16px' }}>
                ...
                <Button
                    ...
                    size="md"
                    radius={8}
                    onClick={() => openCompletedSOItemsModal()}
                >
                    Add Item
                </Button>
            </Group>
            ...
            >
                <Group justify="space-between" align="center">
                    ...
                    <Group gap="md" align="center">
                        ...
                        <Text size="sm" c={customStyles.colors._909090}>
                            Showing {(pagination.pageIndex * pagination.pageSize) + 1} to {Math.min((pagination.pageIndex + 1) * pagination.pageSize, draftedDeliveryOrderItemsCount)} of {draftedDeliveryOrderItemsCount} entries
                        </Text>
                    </Group>
                </Group>
            </Box>
...
export default memo(AddDeliveryOrderDraftedTable);